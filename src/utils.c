...
void save_buffer_to_file(void *buf, uint64_t size, struct filepath *filepath) {
    ...
    /* Set large buffer for better write performance */
    unsigned char *file_buffer = NULL;
    if (size > 1024 * 1024) {
        file_buffer = malloc(16 * 1024 * 1024); /* 16MB buffer for large files */
        if (file_buffer != NULL) {
            setvbuf(f_out, (char*)file_buffer, _IOFBF, 16 * 1024 * 1024);
        }
    }

    if (fwrite(buf, 1, size, f_out) != size) {
...
void save_file_section(FILE *f_in, uint64_t ofs, uint64_t total_size, filepath_t *filepath) {
    ...
    /* Set large buffer for output file to improve write performance */
    unsigned char *file_buffer = malloc(16 * 1024 * 1024); /* 16MB file buffer */
    if (file_buffer != NULL) {
        setvbuf(f_out, (char*)file_buffer, _IOFBF, 16 * 1024 * 1024);
    }

    uint64_t read_size = 0x20000000; /* 512 MB buffer for maximum performance. */
