...
{
    printf("DEBUG: cnmt_gamecard_process() - Starting\n");
    printf("DEBUG: NCA count: %d, Title ID: %016llx\n", 
           cnmt_ctx->nca_count, (unsigned long long)cnmt_ctx->title_id);
    ...
    printf("DEBUG: Processing %d NCA files...\n", cnmt_ctx->nca_count);
    // Process NCA files
    nca_ctx_t nca_ctx;
    filepath_t filepath;
    char *filename;
    ...
    {
        printf("DEBUG: [NCA %d/%d] Starting processing...\n", index + 1, cnmt_ctx->nca_count);
        ...
        printf("DEBUG: [NCA %d/%d] Opening: %s\n", index + 1, cnmt_ctx->nca_count, filepath.char_path);
        free(filename);
        ...
        printf("DEBUG: [NCA %d/%d] Calling nca_gamecard_process()...\n", index + 1, cnmt_ctx->nca_count);
        nca_gamecard_process(&nca_ctx, &filepath, index, cnmt_xml_ctx, cnmt_ctx, nsp_ctx);

        printf("DEBUG: [NCA %d/%d] Freeing contexts...\n", index + 1, cnmt_ctx->nca_count);
        nca_free_section_contexts(&nca_ctx);
        printf("DEBUG: [NCA %d/%d] Completed successfully\n", index + 1, cnmt_ctx->nca_count);
    }

    printf("DEBUG: Processing meta NCA...\n");
    // Process meta nca
    nca_init(&nca_ctx);
    nca_ctx.tool_ctx = tool;
    ...
    printf("DEBUG: Calling nca_gamecard_process() for meta NCA...\n");
    nca_gamecard_process(&nca_ctx, &cnmt_ctx->meta_filepath, cnmt_ctx->nca_count, cnmt_xml_ctx, cnmt_ctx, nsp_ctx);
    nca_free_section_contexts(&nca_ctx);
    printf("DEBUG: Meta NCA processed\n");

    // Create .cnmt.xml
    printf("\n");
    cnmt_create_xml(cnmt_xml_ctx, cnmt_ctx, nsp_ctx);

    // Debug: Print title name before using it
    printf("DEBUG CNMT: nsp_ctx->title_name = '%s'\n", nsp_ctx->title_name);
    printf("DEBUG CNMT: tool->settings.titlename = %d\n", tool->settings.titlename);
    printf("DEBUG CNMT: cnmt_ctx->type = 0x%02x\n", cnmt_ctx->type);

    // Set NSP filename
    char *nsp_filename;
    if (cnmt_ctx->type == 0x80 && tool->settings.titlename == 1) // Application
    {
        printf("DEBUG CNMT: Using title name for application NSP filename\n");
        // nsp filename = titlename + .nsp
        if (strlen(nsp_ctx->title_name) == 0)
        {
            fprintf(stderr, "ERROR: title_name is empty but titlename flag is set!\n");
            fprintf(stderr, "       Falling back to title ID\n");
        ...
        else
        {
            ...
            printf("DEBUG CNMT: NSP filename will be: %s.nsp\n", nsp_filename);
        }
    }
    ...
    printf("\n");
    printf("DEBUG: cnmt_gamecard_process() - About to call nsp_create()...\n");
    nsp_create(nsp_ctx);
    printf("DEBUG: cnmt_gamecard_process() - nsp_create() returned successfully\n");
    printf("DEBUG: cnmt_gamecard_process() - Function completing, about to return\n");
}
